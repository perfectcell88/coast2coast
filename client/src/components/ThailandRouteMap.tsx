...
// ─── Coastline & Land Paths (Kept as provided) ─────────────────────────────
const GULF_COAST = "M 215,48 C 220,72 224,98 226,125 C 228,152 226,175 222,198 C 218,220 210,242 200,260 C 193,272 186,280 178,286";
const ANDAMAN_COAST = "M 72,100 C 74,124 77,148 82,172 C 86,192 90,208 94,224 C 97,238 99,248 100,258 C 102,274 106,294 112,314 C 118,334 126,354 134,372 C 138,384 140,396 140,408";
...
const PHUKET_ISLAND = "M 58.8,351.8 L 60.2,352.7 L 57.6,353.9 L 57.4,356.6 L 54.0,355.6 L 52.6,358.9 L 50.6,358.6 L 50.2,351.8 L 48.2,350.5 L 50.0,346.5 L 50.0,336.4 L 53.1,337.6 L 54.0,340.9 L 60.1,342.2 L 57.4,348.2 L 58.8,351.8 Z";

// ─── Constants ─────────────────────────────────────────────────────────────
const LAND_BRIDGE = "M 107.5,213.7 L 71.0,242.5";
const SINGAPORE_EXTENSION = "M 57.1,353.0 C 45,390 50,440 65.0,470";
...
const ALL_PINS: Record<string, { x: number; y: number; label: string; side: "left" | "right" }> = {
  ...
  singapore:{ x: 65.0,  y: 470.0, label: "Singapore",side: "right" },
};
...
const ROUTES_DATA = [
  {
    ...
    singaporeSea: SINGAPORE_EXTENSION,
    originPin: "pattaya",
    destPin: "singapore",
...
export default function ThailandRouteMap({ activeRoute, onPrev, onNext, totalRoutes }: any) {
  ...
  useEffect(() => { setAnimKey((k) => k + 1); }, [activeRoute]);

  const route = ROUTES_DATA[activeRoute] || ROUTES_DATA[0];
  const isForward = route.direction === "forward";
  const visiblePins = Array.from(new Set(["chumphon", "ranong", route.originPin, route.destPin]));
  const shouldAnimate = inView;

  return (
    <div ref={ref} className="relative w-full select-none" style={{ maxWidth: 520, margin: "0 auto" }}>
      {/* Navigation and Indicator Dots (Restored from your code) */}
      {onPrev && <button onClick={onPrev} className="absolute z-20 left-[10px] top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-[rgba(0,200,200,0.15)] border border-[rgba(0,200,200,0.35)] backdrop-blur-md flex items-center justify-center text-[#00c8c8]"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 2L4 7L9 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>}
      {onNext && <button onClick={onNext} className="absolute z-20 right-[10px] top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-[rgba(0,200,200,0.15)] border border-[rgba(0,200,200,0.35)] backdrop-blur-md flex items-center justify-center text-[#00c8c8]"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M5 2L10 7L5 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>}

      <AnimatePresence mode="wait">
        <motion.div key={animKey} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <svg viewBox="0 0 320 480" className="w-full h-auto overflow-visible rounded-2xl bg-[#041828]">
            <defs>
              <marker id="arrow-teal" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" fill="#00c8c8" /></marker>
              <filter id="glow-teal"><feGaussianBlur stdDeviation="3.5" /><feMerge><feMergeNode /><feMergeNode in="SourceGraphic" /></feMerge></filter>
            ...
            <path d={PHUKET_ISLAND} fill="url(#land-grad)" stroke="#2a5a3a" strokeWidth="1" />
            ...
            {shouldAnimate && (
              <>
                {/* Gulf Leg */}
                {route.gulfSea && (
                  <motion.path d={route.gulfSea} fill="none" stroke="#00c8c8" strokeWidth="2.5" strokeDasharray="8 5" variants={drawPath(0, 1.6)} initial="hidden" animate="visible" filter="url(#glow-teal)" />
                )}
                
                {/* Land Bridge */}
                {route.landBridge && (
                  <motion.path d={route.landBridge} fill="none" stroke="#f59e0b" strokeWidth="2.5" strokeDasharray="5 4" variants={drawPath(1.4, 0.8)} initial="hidden" animate="visible" />
                )}

                {/* Andaman Leg */}
                {route.andamanSea && (
                  <motion.path d={route.andamanSea} fill="none" stroke="#00c8c8" strokeWidth="2.5" strokeDasharray="8 5" variants={drawPath(1.4, 1.6)} initial="hidden" animate="visible" filter="url(#glow-teal)" />
                )}

                {/* Singapore Extension - FIXED & ANIMATED */}
                {route.singaporeSea && (
                  <motion.path 
                    d={route.singaporeSea} 
                    fill="none" 
                    stroke="#00c8c8" 
                    strokeWidth="2.5" 
                    strokeDasharray="8 5" 
                    variants={drawPath(2.8, 1.2)} // Delays until Andaman leg finishes
                    initial="hidden" 
                    animate="visible" 
            ...
            {visiblePins.map((pinId, i) => {
              ...
              if (!pin) return null;
              const isAccent = pinId === route.originPin || pinId === route.destPin;
              const color = pinId === "chumphon" || pinId === "ranong" ? "#f59e0b" : isAccent ? "#00c8c8" : "#94a3b8";

              return (
                <motion.g key={pinId} initial="hidden" animate="visible" variants={popIn(0.3 + i * 0.18)}>
                  <circle cx={pin.x} cy={pin.y} r={isAccent ? 5 : 4} fill={color} />
                  <text x={pin.side === "right" ? pin.x + 12 : pin.x - 12} y={pin.y + 4} fontSize="10" fontWeight="bold" fill={color} textAnchor={pin.side === "right" ? "start" : "end"} style={{ textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}>
                    {pin.label}
                  </text>
                </motion.g>
              );
