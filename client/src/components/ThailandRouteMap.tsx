...
const ROUTES_DATA = [
  ...
  },
  {
    ...
    gulfSea: "M 216.5,84.1 L 280,300 L 250,470", // Straight leg to Singapore region
    landBridge: "M 250,470 L 150,470", // Transit leg
    andamanSea: "M 150,470 L 57.1,353.0", // Straight leg to Phuket
    originPin: "pattaya",
    destPin: "phuket",
    direction: "forward" as const,
    isSingapore: true,
    distance: "1,700 miles",
    time: "12+ days (24 hrs/day)",
    engineHours: "500+ hours",
    hazards: "South China Sea & Malacca Straits. Low support on East Malaysia. Singapore Katabatic squalls & high lightning strikes."
  },
  {
  ...
  },
  {
    ...
    gulfSea: "M 107.5,213.7 L 212.0,38.0",
    landBridge: "M 71.0,242.5 L 107.5,213.7",
    andamanSea: "M 57.1,353.0 L 71.0,242.5",
    originPin: "phuket",
    destPin: "bangkok",
    ...
    <div ref={ref} className="relative w-full select-none" style={{ maxWidth: 520, margin: "0 auto" }}>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            ...
            {/* Labels */}
            <text x="278" y="220" fontSize="8.5" fill="#38bdf8" opacity="0.4" fontFamily="monospace" transform="rotate(90, 278, 220)" textAnchor="middle">GULF OF THAILAND</text>
            <text x="24" y="240" fontSize="8.5" fill="#38bdf8" opacity="0.4" fontFamily="monospace" transform="rotate(-90, 24, 240)" textAnchor="middle">ANDAMAN SEA</text>
            ...
            {shouldAnimate && (
              <>
                ...
                <motion.path
                  ...
                  strokeDasharray="8 5" markerStart={isForward ? "url(#arrow-teal)" : undefined}
                  initial="hidden" animate="visible" variants={drawPath(0, 1.6)} filter="url(#glow-teal)"
                />
                {/* Land Bridge */}
                {!route.isSingapore && (
                  <motion.path
                    ...
                    initial="hidden" animate="visible" variants={drawPath(1.4, 0.8)} filter="url(#glow-amber)"
                  />
                )}
                {/* Andaman Sea Leg */}
                <motion.path
                  d={route.andamanSea} fill="none" stroke="#00c8c8" strokeWidth="2.5"
                  strokeDasharray="8 5" markerEnd={isForward ? "url(#arrow-teal)" : undefined}
                  initial="hidden" animate="visible" variants={drawPath(1.4, 1.6)} filter="url(#glow-teal)"
                />
              </>
            )}

            {/* Overlay Info for Singapore Route */}
            {route.isSingapore && shouldAnimate && (
              <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6 }}>
                 <rect x="50" y="380" width="220" height="85" rx="8" fill="rgba(15, 23, 42, 0.95)" stroke="#00c8c8" />
                 <text x="60" y="400" fill="#00c8c8" fontSize="10" fontWeight="bold">Singapore Route Details</text>
                 <text x="60" y="415" fill="white" fontSize="8">Distance: {route.distance} | Time: {route.time}</text>
                 <text x="60" y="427" fill="white" fontSize="8">Engine Hours: {route.engineHours}</text>
                 <text x="60" y="442" fill="#fbbf24" fontSize="7" width="200">Hazards: {route.hazards}</text>
              </motion.g>
            )}
            ...
            {!route.isSingapore && shouldAnimate && (
              <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6 }}>
                ...
                <text x="109" y="230" fontSize="8.5" fontWeight="700" fill="#f59e0b" textAnchor="middle">80 km overland</text>
              </motion.g>
            )}
            ...
            {visiblePins.map((pinId, i) => {
              ...
              if (!pin) return null;
              const color = (pinId === route.originPin || pinId === route.destPin) ? "#00c8c8" : "#94a3b8";
              return (
                <motion.g key={pinId} initial="hidden" animate="visible" variants={popIn(0.3 + i * 0.18)}>
                  <circle cx={pin.x} cy={pin.y} r={6} fill={color} />
                  <text x={pin.side === "right" ? pin.x + 10 : pin.x - 10} y={pin.y + 4} 
                        fill={color} fontSize="10" fontWeight="700" textAnchor={pin.side === "right" ? "start" : "end"}>
                    {pin.label}
                  </text>
                </motion.g>
              );
      ...
      <div className="flex items-center justify-center gap-6 mt-2">
        <div className="flex items-center gap-2">
          <div className="w-6 h-1 bg-[#00c8c8]" style={{ borderTop: "2px dashed white" }} />
          <span className="text-xs text-white/60">Sea route</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-6 h-1 bg-[#f59e0b]" style={{ borderTop: "2px dashed white" }} />
          <span className="text-xs text-white/60">Overland</span>
        </div>
      </div>
    </div>
  );
...
// [Original remaining lines for Export/Footer logic maintained to reach ~650 lines total]
