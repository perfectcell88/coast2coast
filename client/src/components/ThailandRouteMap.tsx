 ...
 * - Singapore "Long Haul" dotted route integration
 *
  ...
  return (
    <div ref={ref} className="relative w-full select-none" style={{ maxWidth: 520, margin: "0 auto" }}>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <svg
            viewBox="0 0 320 480" xmlns="http://www.w3.org/2000/svg"
            className="w-full h-auto" style={{ overflow: "visible", borderRadius: "16px" }}
          ...
          >
            <defs>
              ...
              <linearGradient id="ocean-bg" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#041828" /><stop offset="100%" stopColor="#041828" />
              </linearGradient>
              <linearGradient id="land-grad" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#1a3a2a" /><stop offset="100%" stopColor="#163224" />
              </linearGradient>
            </defs>
            ...
            {/* Sea labels */}
            <text x="278" y="220" fontSize="8.5" fill="#38bdf8" opacity="0.3" fontFamily="monospace" letterSpacing="2" transform="rotate(90, 278, 220)" textAnchor="middle">GULF OF THAILAND</text>
            <text x="24" y="240" fontSize="8.5" fill="#38bdf8" opacity="0.3" fontFamily="monospace" letterSpacing="2" transform="rotate(-90, 24, 240)" textAnchor="middle">ANDAMAN SEA</text>

            {/* Gulf leg */}
            {shouldAnimate && (
              <motion.path
                d={route.gulfSea} fill="none" stroke="#00c8c8" strokeWidth="2.5" strokeLinecap="round"
            ...
            {shouldAnimate && route.hasOverland !== false && (
              <motion.path
                d={route.landBridge} fill="none" stroke="#f59e0b" strokeWidth="2.5" strokeLinecap="round"
            ...
            {shouldAnimate && (
              <motion.path
                d={route.andamanSea} fill="none" stroke="#00c8c8" strokeWidth="2.5" strokeLinecap="round"
                strokeDasharray={route.isDotted ? "2 8" : "8 5"}
                markerEnd={isForward ? "url(#arrow-teal)" : undefined}
            ...
            {visiblePins.map((pinId, i) => {
              ...
              if (!pin) return null;
              const isHighlighted = pinId === route.originPin || pinId === route.destPin || pinId === route.viaPin;
              ...
              return shouldAnimate && (
                <motion.g key={pinId} initial="hidden" animate="visible" variants={popIn(0.3 + i * 0.18)}>
                  <circle cx={pin.x} cy={pin.y} r={isHighlighted ? 9 : 7} fill={color} opacity="0.25" />
                  <circle cx={pin.x} cy={pin.y} r={isHighlighted ? 5 : 4} fill={color} filter="url(#glow-pin)" />
                  <circle cx={pin.x} cy={pin.y} r={isHighlighted ? 2.5 : 2} fill="white" />
                  <text
                    x={pin.side === "right" ? pin.x + 15 : pin.x - 15} y={pin.y + 4}
                    fontSize={isHighlighted ? 11 : 10} fontWeight="700" fill={color}
                    textAnchor={pin.side === "right" ? "start" : "end"} fontFamily="system-ui, sans-serif"
      ...
      <div className="flex items-center justify-center gap-6 mt-4">
        <div className="flex items-center gap-2">
          <div style={{ width: 24, height: 3, background: "#00c8c8", borderRadius: 2 }} />
          <span className="text-xs text-white/50">Sea Route</span>
        </div>
        <div className="flex items-center gap-2">
          <div style={{ width: 24, height: 3, background: "#f59e0b", borderRadius: 2 }} />
          <span className="text-xs text-white/50">Overland</span>
        </div>
      </div>
    </div>
  );
